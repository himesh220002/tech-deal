...
export default function HeroSection({ activeCategory, setActiveCategory, setSearchTerm, dashboardRef }) {
  ...
  return (
    <section className="text-center py-10 md:py-16 px-4 md:px-20">
      {/* Heading */}
      <h2 className="text-2xl md:text-4xl font-bold">
        Find the Best <span className="text-purple-400">Tech Deals</span>
      </h2>
      <p className="mt-2 md:mt-6 text-gray-300 max-w-2xl mx-auto">
      ...
      {/* Categories */}
      <div className="mt-4 md:mt-6 flex flex-wrap justify-center gap-1 md:gap-3 ">
        {categories.map(({ name, icon: Icon }) => (
          <button
            key={name}
            onClick={() => setActiveCategory(name)}
            className={`flex items-center gap-2 px-3 text-md md:px-4 py-1 md:py-2 rounded-full transition ${activeCategory === name
